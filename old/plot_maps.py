...
### plotting, annotation, and figure generation
...
def annotate( ax, projection=None, line_of_sight=[], line_of_sight_color='k', zenith=[], zenith_color='k', time_delay=[], time_delay_color='k', time_delay_alpha=1.0, marker_Dec_RA=[], marker='o', marker_color='k', marker_size=4, marker_edgewidth=1, marker_alpha=1.0, continents=False, continents_color='k', continents_alpha=1.0 ):
    ...
    if opts.continents:
        ...
        for shape in geojson['geometries']:
            ...
            stack_ax.plot( verts[:, 0], verts[:, 1], color=continents_color, linewidth=0.5, alpha=continents_alpha )
...
### functions that set up data for plotting
...
if __name__=="__main__":
    ...
    line_of_sight = gen_line_of_sigth( opts.line_of_sight, coord=opts.coord, gps=opts.gps )

    ### figure out postions for zenith markers
    zenith = gen_zenith( IFOs, coord=opts.coord, gps=opts.gps )

    ### figure out points for time_delay
    time_delay = gen_time_delay( opts.time_delay_Dec_RA, opts.time_delay, coord=opts.coord, gps=opts.gps, degrees=opts.time_delay_degrees )

    ### figure out points for markers
    marker_Dec_RA = gen_marker_Dec_RA( opts.marker_Dec_RA, coord=opts.coord, gps=opts.gps, degrees=opts.marker_degrees )
    ...
    if opts.stack_posteriors:
        stack_fig, stack_ax = gen_fig_ax( figind, figwidth=opts.figwidth, figheight=opts.figheight, projection=opts.projection )
        figind += 1
        ...
        if opts.stack_posteriors_background:
            ...
            heatmap( post, stack_ax, color_map=opts.color_map )
    ...
    for label in labels:
        ...
        fig, ax = gen_fig_ax( figind, figwidth=opts.figwidth, figheight=opts.figheight, projection=opts.projection )
        figind += 1

        heatmap( post, ax, color_map=opts.color_map )
        annotate( ax,
                  projection          = opts.projection,
                  line_of_sight       = line_of_sight,
                  line_of_sight_color = opts.line_of_sight_color,
